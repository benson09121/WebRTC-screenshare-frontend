...
- **Current result:** Vidking is not remotely controllable from an ordinary cross-origin PairBeam page, so the experimental cross-browser MV3 extension supplies the local player bridge after explicit installation and viewer consent. Desktop Chrome/Chromium uses its service-worker entry and Firefox uses its event-page background entry. Vidsrc.sbs remains disabled because its changing nested origins have not passed the same control and security model.
- **Missing extension:** Disable invitation acceptance and auto-detect the current browser locally. Firefox receives a `web-ext`-built ZIP whose `manifest.json` is at the archive root, plus `about:debugging` / **Load Temporary Add-on** steps that select the ZIP directly; Chrome, Edge, Opera, and Chromium receive the unpacked ZIP plus Developer mode / **Load unpacked** steps; unsupported or mobile browsers receive both desktop alternatives without a false compatibility claim. Explain that PairBeam never initiates installation, Firefox temporary installs end on restart, and standard Firefox requires Mozilla signing for permanent installation. Never distribute a `.crx` or expose a `.pem` signing key.
