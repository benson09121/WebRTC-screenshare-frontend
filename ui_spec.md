...
- **Missing extension:** Disable invitation acceptance and show a prominent unpacked-source ZIP download in incoming, outgoing, and active-session states. Explain that PairBeam never initiates installation. Present extract, Developer mode, **Load unpacked**, folder selection, and reload steps. Never distribute a `.crx` or expose a `.pem` signing key.
