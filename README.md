...
- Includes an experimental Chrome/Chromium and Firefox companion extension that can synchronize a consented Vidking, Zoryva, 2Embed, or VidSrc.io movie or exact TV episode without relaying provider media through PairBeam
...
flowchart LR
    ...
    A -. local provider request .-> P[Selected provider embed]
...
## Test experimental provider synchronization
...
3. Choose **Load unpacked** and select the extracted folder that directly contains `manifest.json`.
...
After installation, join the same room and open **Share a movie → Browse catalog**. First choose Vidking, Zoryva, 2Embed, or VidSrc.io; then select a movie, or select a TV series followed by an exact season and episode. The provider ID is included in the validated invitation, so both participants load the same source only after the other participant reviews the third-party disclosure and accepts.
...
TV proposals include the provider ID, TMDB series ID, exact season and episode numbers, and episode title. Each participant independently builds the selected provider's documented exact-episode route; PairBeam sends only the validated identity and synchronized playback state.
