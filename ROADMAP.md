...
- [x] **P0 — Build a cross-browser Vidking companion spike.** Desktop Chrome/Chromium and Firefox load an exact movie or TV episode locally after consent; proposer-authoritative play/pause/seek state travels through the existing data channel. One auditable MV3 source declares both Chrome's service worker and Firefox's event-page background entry.
