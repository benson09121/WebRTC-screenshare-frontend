...
- [x] **P0 — Build a Chrome/Vidking companion spike.** Both browsers load an exact movie or TV episode locally after consent; proposer-authoritative play/pause/seek state travels through the existing data channel. Packed personal distribution and the unpacked fallback are clearly labeled experimental.
...
- [x] **P0 — Make the companion-extension requirement actionable.** If PairBeam cannot detect the extension, invitations cannot be accepted and every proposal/session state shows a prominent prompt with a direct download of the personally distributed `extension.crx`, concise drag-to-install steps, and a detailed setup link. Both participants are told to reload the room after installation.
- [x] **P0 — Package the personal extension download.** Bundle the packed `.crx` as a non-inlined Vite asset and keep its `.pem` signing key private, ignored by Git, and outside the deployed download flow. A Chrome Web Store listing is intentionally not required for this personal project.
...
- [ ] **P0 — Complete the physical-device series test matrix.** Test at least one multi-season series on two physical devices, including episode identity, season changes, autoplay denial, subtitles, fullscreen, popup behavior, drift, reconnect, extension restart, and provider shutdown. Vidsrc remains disabled.
...
- **Current experiment:** Vidking remains unavailable to an ordinary PairBeam page because of the same-origin boundary and missing inbound player API. A personally distributed packed Chrome MV3 companion extension now supplies the missing local control bridge for explicit testing, with the unpacked directory retained as a fallback. This is not a media extractor: each user requests the same public embed directly, while PairBeam carries only playback state.
...
- [x] Replace the fullscreen control strip with a persistent compact arrow handle. It opens the mic, camera, screen-share, movie, and leave dashboard above the media. The top-right fullscreen/stop actions and left episode trigger remain independently available and fade to a quiet but clickable state after inactivity instead of being removed. Settings popovers close with the dashboard, and reduced-motion preferences are respected.
