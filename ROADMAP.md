...
- [x] **P0 — Build a cross-browser multi-provider companion spike.** Desktop Chrome/Chromium and Firefox can load an exact Vidking, Zoryva, 2Embed, or VidSrc.io movie/episode locally after consent; proposer-authoritative play/pause/seek state travels through the existing data channel. One auditable runtime source is packaged with separate Chrome service-worker and Firefox event-page manifests.
- [x] **P0 — Require provider selection before catalog search.** The catalog opens with an accessible provider picker. The chosen provider ID is schema-validated, included in the peer proposal and every episode revision, disclosed before acceptance, and used by both browsers to derive the same provider URL.
- [x] **P0 — Limit multi-provider extension access to reviewed origins.** Extension 0.5.1 lists only the four provider roots and their currently observed nested player origins. Frames report ready only after a real video element is found. PairBeam does not request `<all_urls>` access; a rotated VidSrc.io player host requires review and a new package.
- [x] **P0 — Detect stale companion extensions.** The page and extension exchange the installed package version. A pre-0.5.1 build is treated as update-required, keeps invitation acceptance disabled, and offers the browser-specific ZIP instead of waiting indefinitely for a provider it cannot control.
...
- [ ] **P0 — Complete the physical-device provider test matrix.** Test a movie and multi-season series for every provider on two physical devices and cover Chrome↔Chrome, Firefox↔Firefox, and mixed Chrome↔Firefox sessions, including episode identity, season changes, autoplay denial, subtitles, fullscreen, popup behavior, drift, reconnect, extension/background restart, nested-host rotation, and provider shutdown.
...
- Zoryva documents `/embedded/movie/{tmdbId}` and `/embedded/tv/{tmdbId}/{season}/{episode}`. 2Embed documents `/embed/{id}` and `/embedtv/{id}&s={season}&e={episode}`. VidSrc.io documents `/embed/movie/{id}` and `/embed/tv/{id}/{season}/{episode}` plus outbound player events, but not a stable inbound command API.
...
- **Current experiment:** A manually loaded cross-browser MV3 companion extension supplies the missing local control bridge for explicit Chrome/Chromium and Firefox testing. The provider-first picker supports Vidking, Zoryva, 2Embed, and VidSrc.io. PairBeam distributes auditable browser-labelled ZIPs and auto-selects the matching manual setup flow. This is not a media extractor: each user requests the selected public embed directly, while PairBeam carries only playback state.
- **VidSrc boundary:** Vidsrc.sbs remains disabled in the ordinary adapter audit. The distinct VidSrc.io extension route is experimental and limited to its root plus the nested player hostname observed during the 2026-08-26 audit. Host rotation intentionally fails closed until the manifest is reviewed and repackaged.
...
Research: [Vidking](https://www.vidking.net/), [Zoryva embed API](https://zoryva.me/Api/Embedded), [2Embed](https://www.2embed.cc/), [VidSrc.io embed API](https://vidsrc.io/vidsrc/docs), [Vidsrc.sbs embed documentation](https://vidsrc.sbs/), [browser same-origin policy](https://developer.mozilla.org/en-US/docs/Web/Security/Defenses/Same-origin_policy), [`postMessage` security](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage), [iframe security/permissions](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/iframe), [Mozilla cross-browser MV3 backgrounds](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/manifest.json/background), and [Firefox temporary installation](https://extensionworkshop.com/documentation/develop/temporary-installation-in-firefox/).
...
- [x] **P1 — Keep catalog metadata separate from playback availability.** A TMDB result means “metadata found,” not “stream available.” Ordinary cross-origin providers remain blocked by the capability gate; only an explicit extension provider selection may create a proposal, and the peer must consent before either browser loads the provider iframe.
...
- [x] **P1 — Replace the overlay collection with a responsive room shell.** Camera calls use an equal tile grid; shared screens, movies, and providers use a shared-content stage with a labeled participant dock below it. Desktop chat reserves a 340px rail in every room mode, while narrow screens use a full-width right drawer without horizontal overflow.
...
- [x] **P1 — Add a composer emoji picker.** A full searchable Unicode picker inserts emoji at the caret, works for message reactions, supports keyboard navigation and Escape, and portals above the chat surface.
- [x] **P1 — Keep chat unobtrusive during media playback.** Desktop chat consistently reserves a docked rail for camera, native-share, and external-provider modes; narrow screens use an overlay drawer. The launcher avoids provider controls, an open fullscreen chat never disappears on idle, and chat activity does not wake PairBeam's media controls.
...
- [x] Add a bounded connection-recovery watchdog. Initial negotiation and ICE-restart offers now time out after 10 seconds, retry instead of leaving the in-flight guard stuck after a lost answer, and rebuild the full peer transport on the final automatic attempt.
- [x] Stop showing “Call reconnecting” indefinitely. Exhausted recovery now becomes an explicit **Call unavailable** state with reload/TURN guidance, while an active external-watch provider remains mounted during the automatic transport rebuild.
...
- [x] Replace the fullscreen overlay with a compact docked chat rail so the shared content remains visible; use a full-width right drawer on narrow screens.
...
- [x] Coordinate fullscreen inactivity across the media chrome while keeping chat independent. Pointer or keyboard activity on the stage and forwarded provider activity reset the three-second timer; interaction inside an open chat does not wake unrelated player controls, and the chat itself remains open until dismissed.
- [x] Separate participants from selected screen and external-provider playback with a compact bottom dock. The dock keeps both labeled camera/avatar tiles visible in standard view; focus mode hides it deliberately, while desktop PiP remains a separate explicit action.
- [x] Make the media dock the Discord-style view selector. Remove the duplicate `Participant` / `Your screen` toolbar; clicking `Participant`, `You`, a native share/movie, or the synchronized-provider tile focuses it locally. Mark the selected tile, keep provider playback mounted while a camera is focused, place secondary actions in top-right ellipsis menus, lower the dock, position the call dashboard directly above it, and use a down/up arrow pair to enter or leave content-only focus mode.
- [x] Give every shared-content source a Discord-style unobstructed viewport. When the dock is visible, place native screen shares, uploaded/direct-link movies, focused cameras, and external provider iframes inside a rounded, responsive inset with a black top/side gutter so the complete frame clears the dashboard and participant tiles. Measure movie fitting from the reduced viewport and restore the full stage only in explicit focus mode.
- [x] Rebind participant camera media when external playback starts and render the participant dock above the provider iframe without covering the provider's playback surface.
...
- [x] Remove the competing fullscreen dashboard handle. The dashboard fades directly and returns with activity, while the view dock and Chat launcher stay reachable until explicit focus mode. Keep the single focus arrow at the dock's trailing edge so it cannot overlap dashboard settings.
- [x] Finalize the shared-stage vertical hierarchy: hide the room code after peer connection, keep shared content above the participant/source dock, place a smaller call dashboard at the safe bottom edge, and consolidate stage actions into one top-right ellipsis menu.
- [x] Move connection health into the dashboard as an accessible three-bar control: three green bars for good, two amber for limited, one red for poor, and gray pulsing bars while measuring. Keep detailed path, protocol, latency, loss, bitrate, video, and limitation data in its Popover.
- [x] Add ephemeral one-level chat replies. Validate and send the referenced message ID through the existing peer data channel, show a cancellable composer preview, render a quoted source, and support jump-to-message highlighting without a database.
- [x] Fix installed-extension false negatives. Let the PairBeam bridge announce itself before the Chromium MV3 background worker responds, show a checking state before install guidance, and place Chromium `manifest.json` at the downloaded ZIP root for an unambiguous Load unpacked workflow.
...
- [x] **P2 — Add ephemeral image and GIF sending.** Local images are resized and compressed into bounded peer-to-peer data-channel attachments; GIPHY search sends structured HTTPS URLs with bounded alt text and lazy-loaded previews. PairBeam does not proxy or persist either type. GIF search requires a public, origin-restricted `VITE_GIPHY_API_KEY`.
