...
- [x] **P0 — Build a Chrome/Vidking companion spike.** Both browsers load an exact movie or TV episode locally after consent; proposer-authoritative play/pause/seek state travels through the existing data channel. Manual installation from a downloadable unpacked-source ZIP is clearly labelled experimental.
...
- [x] **P0 — Make the companion-extension requirement actionable.** If PairBeam cannot detect the extension, invitations cannot be accepted and every proposal/session state shows a prominent unpacked-source ZIP download with extract, Developer mode, **Load unpacked**, folder-selection, and reload instructions.
...
- **Current experiment:** Vidking remains unavailable to an ordinary PairBeam page because of the same-origin boundary and missing inbound player API. A manually loaded Chrome MV3 companion extension now supplies the missing local control bridge for explicit testing. PairBeam distributes its auditable source as a ZIP; each user extracts it and selects the folder with **Load unpacked**. This is not a media extractor: each user requests the same public embed directly, while PairBeam carries only playback state.
