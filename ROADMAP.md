...
- [x] Give every shared-content source a Discord-style unobstructed viewport. When the dock is visible, slightly reduce native screen shares, uploaded/direct-link movies, focused cameras, and external provider iframes so the complete frame clears the dashboard and participant tiles. Measure movie fitting from the reduced viewport and restore the full stage only in explicit focus mode.
