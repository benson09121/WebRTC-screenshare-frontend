...
const PUBLISH_EVENTS = new Set(['play', 'pause', 'seeked', 'timeupdate', 'ready', 'canplay', 'command-applied']);
...
const episodeLabel = media => media?.mediaType === 'tv'
    ...
    <div className="flex min-w-0 items-center gap-3">
      <div className="grid h-16 w-11 shrink-0 place-items-center overflow-hidden rounded-lg bg-white/[0.06] text-zinc-600">
        {posterUrl(media?.posterPath)
          ? <img src={posterUrl(media.posterPath)} alt="" className="size-full object-cover" />
          : <Film className="size-4" aria-hidden="true" />}
      </div>
      <div className="min-w-0">
        <p className="truncate text-sm font-semibold text-zinc-100">{media?.title}</p>
        <p className="mt-1 truncate text-xs text-zinc-400">{selectedEpisode || 'Vidking sync prototype · no media relay'}</p>
        {selectedEpisode ? <p className="mt-0.5 text-[10px] text-zinc-600">Vidking sync prototype · no media relay</p> : null}
      </div>
    </div>
  );
};
...
export default function ExternalWatchParty({ isIdle }) {
  ...
  const sendBridgeCommand = useCallback(command => {
    window.postMessage({ channel: PAGE_CHANNEL, type: 'command', command }, window.location.origin);
  }, []);
  ...
  const scheduleExternalResume = useCallback(commandId => {
    window.clearTimeout(externalSeekResumeTimerRef.current);
  ...
  const toggleExternalFullscreen = useCallback(async () => {
    try {
      if (document.fullscreenElement) {
        ...
        await (document.getElementById('root') || document.documentElement).requestFullscreen();
      }
    } catch (error) {
      setPlayerError(error?.message || 'Fullscreen could not be opened in this browser.');
    }
  }, []);
  ...
  useEffect(() => {
    const onMessage = event => {
      ...
      if (['attached', 'ready', 'canplay'].includes(playerEvent.event)) setPlayerReady(true);
      if (playerEvent.event === 'error') {
        setPlayerError(playerEvent.error || 'The provider player rejected a synchronized command.');
        return;
      }
      const session = sessionRef.current;
      if (!session || typeof playerEvent.paused !== 'boolean' || !Number.isFinite(playerEvent.position)) return;

      if (session.authority === 'local' && PUBLISH_EVENTS.has(playerEvent.event)) {
        const now = performance.now();
        if (playerEvent.event === 'timeupdate' && now - lastPublishedAtRef.current < 1500) return;
      ...
      if (
        ...
        requestExternalWatchControl({
          ...
          position: playerEvent.position,
          resumeAfterSeek: playerEvent.event === 'seeked' && playerEvent.resumeAfterSeek === true,
        });
      }
    };

    window.addEventListener('message', onMessage);
    const ping = () => window.postMessage({ channel: PAGE_CHANNEL, type: 'ping' }, window.location.origin);
    ping();
  ...
  }, [clearExternalSeekTimers, publishExternalWatchState, requestExternalWatchControl, scheduleExternalResume]);

  useEffect(() => {
    const active = Boolean(activeWatchProposalId);
    window.postMessage({ channel: PAGE_CHANNEL, type: 'watch-session', active }, window.location.origin);
    return () => {
      if (active) {
        window.postMessage({ channel: PAGE_CHANNEL, type: 'watch-session', active: false }, window.location.origin);
      }
    };
  }, [activeWatchProposalId]);

  useEffect(() => {
    if (!externalWatchCommand || lastAppliedCommandRef.current === externalWatchCommand.commandId) return;
  ...
  }, [clearExternalSeekTimers, externalWatchCommand, scheduleExternalResume, sendBridgeCommand]);
  ...
  if (externalWatchInvite) {
    return (
      <div className="fixed inset-0 z-[90] grid place-items-center bg-black/70 p-4 backdrop-blur-sm" data-idle-exempt="true">
        <section className="w-full max-w-md rounded-2xl border border-white/10 bg-[#111719] p-5 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="watch-invite-title">
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-teal-300">Watch together invite</p><h2 id="watch-invite-title" className="mt-1 text-lg font-semibold text-white">Open this title together?</h2></div>
            <Button variant="ghost" size="icon" onClick={() => respondExternalWatchProposal(false)} aria-label="Decline invitation"><X className="size-4" /></Button>
          </div>
  ...
  if (!externalWatchSession) {
    if (!['declined', 'cancelled'].includes(externalWatchProposalStatus)) return null;
    return <div className="fixed left-1/2 top-6 z-[90] flex -translate-x-1/2 items-center gap-3 rounded-xl border border-white/10 bg-[#111719]/95 px-4 py-3 text-sm text-zinc-300 shadow-2xl" role="status"><span>{externalWatchProposalStatus === 'declined' ? 'The watch invite was declined.' : 'The watch invite was cancelled.'}</span><Button variant="ghost" size="icon" onClick={stopExternalWatch} aria-label="Dismiss"><X className="size-4" /></Button></div>;
  }
  ...
  return (
    <section className={`absolute inset-0 z-10 overflow-hidden bg-black ${isChatOpen && isFullscreen && !isIdle ? 'external-watch--chat-docked' : ''}`} aria-label={`Watching ${externalWatchSession.media.title}`} data-idle-exempt="true">
      ...
      >
          ...
          <Button variant="secondary" size="sm" onClick={stopExternalWatch}><X className="size-4" />Stop watching</Button>
      </div>
