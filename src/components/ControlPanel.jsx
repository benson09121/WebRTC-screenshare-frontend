...
const WatchCatalog = React.lazy(() =>
  import('./WatchCatalog').then((module) => ({ default: module.WatchCatalog })),
);

const QUALITY_PRESETS = {
  auto: { label: 'Auto (recommended)', auto: true },
  lossless: {
    label: 'Native resolution (up to 60fps)',
    lossless: true,
    frameRate: 60,
    bitrate: 12000000,
  },
...
const SCREEN_AUDIO_COPY = {
  ...
  monitor:
    'Desktop audio is included from the selected Linux monitor. Use headphones to reduce call-audio echo.',
  unavailable:
    'No screen-audio track was provided. Video is still being shared.',
  direct:
    'Each participant is playing the direct link locally; movie audio is not relayed through WebRTC.',
};

const SEEK_SETTLE_DELAY_MS = 900;

const isMonitorSource = (device) =>
  /(^|[\s._-])monitor([\s._-]|$)|monitor of/i.test(device.label);
const isVirtualAudioSource = (device) =>
  /virtual.?cable|loopback|null.?sink/i.test(device.label);
...
const formatVideoMetrics = (metrics) => {
  ...
  const fps =
    metrics.framesPerSecond == null
      ? ''
      : ` · ${Math.round(metrics.framesPerSecond)}fps`;
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex items-center gap-2.5">
        <VolumeIcon
          className={`size-4 shrink-0 ${muted ? 'text-zinc-600' : 'text-teal-300'}`}
          aria-hidden="true"
        />
        <input
          id={id}
      ...
      <p
        id={`${id}-description`}
        className="mt-1.5 text-[10px] leading-4 text-zinc-600"
      >
        {description}
      </p>
    </div>
...
export const ControlPanel = ({ isIdle }) => {
  ...
  const [autoQualityIndex, setAutoQualityIndex] = useState(
    AUTO_QUALITY_START_INDEX,
  );
  ...
  const [selectedDesktopAudioDevice, setSelectedDesktopAudioDevice] =
    useState('');
  const [screenAudioStatus, setScreenAudioStatus] = useState('idle');
  const [selectedMovie, setSelectedMovie] = useState(null);
  const [, setMovieProgress] = useState({
    currentTime: 0,
    duration: 0,
    isPlaying: false,
  });
  ...
  const [selectedNativeSubtitleTrack, setSelectedNativeSubtitleTrack] =
    useState(null);
  ...
  const clearMovieSource = () => {
    ...
    if (movieObjectUrlRef.current)
      URL.revokeObjectURL(movieObjectUrlRef.current);
  ...
  const startCamera = async () => {
    try {
      ...
      showMediaError(
        err.name === 'NotAllowedError' ? 'Permission Denied' : err.message,
      );
    }
  };
  ...
  useEffect(
    () => () => {
      activeDisplayStreamRef.current
        ?.getTracks()
  ...
  const getDevices = async () => {
    try {
      ...
      setSelectedDesktopAudioDevice((current) =>
        current && !monitorInputs.some((device) => device.deviceId === current)
          ? ''
          : current,
      );
      ...
      if (localStream) {
        ...
        if (activeAudioTrack) {
          const activeDevice = audioInputs.find(
            (d) => d.label === activeAudioTrack.label,
          );
        ...
        if (activeVideoTrack) {
          const activeDevice = videoInputs.find(
            (d) => d.label === activeVideoTrack.label,
          );
  ...
  const handleVideoDeviceChange = async (event) => {
    ...
    try {
      ...
      await updateCameraStream(
        new MediaStream([...audioTracks, newVideoTrack]),
      );
  ...
  const applyScreenQuality = async (
    preset,
    nextContentType = contentTypeRef.current,
  ) => {
    ...
    const contentHint =
      nextContentType === 'movie' ? 'motion' : nextContentType;
    ...
    if (videoTrack.applyConstraints) {
      ...
      try {
        ...
        console.warn(
          'The browser could not apply the requested capture constraints',
          error,
        );
      }
    }
    ...
    setScreenMetrics((current) =>
      sameVideoMetrics(current.capture, capture)
        ? current
        : { ...current, capture },
    );
    ...
    try {
      ...
      const scaleByWidth =
        !preset.lossless && capture.width && preset.width
          ? capture.width / preset.width
          : 1;
      const scaleByHeight =
        !preset.lossless && capture.height && preset.height
      ...
      console.warn(
        'The browser could not apply all screen sender parameters',
        error,
      );
    } finally {
  ...
  useEffect(() => {
    if (!isScreenSharing || localShareSource?.deliveryMode === 'direct')
      return undefined;
    ...
    const sampleScreenSender = async () => {
      ...
      try {
        ...
        if (screenQualitySenderRef.current !== sender) {
          ...
          const currentPreset =
            qualityRef.current === 'auto'
              ? getAutoQualityPreset(
                  adaptiveContentType,
                  autoQualityStateRef.current.index,
                )
        ...
        const summary = summarizeScreenSenderStats(
          report,
          screenStatsSamplesRef.current,
        );
        ...
        const adaptiveContentType =
          localShareSource?.kind === 'movie' ? 'movie' : contentTypeRef.current;
        const profiles =
          AUTO_QUALITY_PROFILES[adaptiveContentType] ||
          AUTO_QUALITY_PROFILES.motion;
  ...
  const stopScreenShare = async ({ preserveMovieSource = false } = {}) => {
    ...
    try {
      ...
      showMediaError(
        'The share stopped, but one outgoing media sender could not be reset',
      );
    } finally {
  ...
  const toggleScreenShare = async () => {
    ...
    try {
      ...
      setScreenMetrics({
        capture: readCaptureMetrics(videoTrack),
        outbound: null,
      });
      screenStatsSamplesRef.current = new Map();
      ...
      if (!screenAudioTrack && selectedDesktopAudioDevice) {
        try {
          ...
          console.warn(
            'The selected desktop-audio monitor could not be captured',
            error,
          );
        }
      }
      ...
      if (!shareStarted) {
        ...
        setSharedContentAudioTrack(null).catch((audioError) => {
          console.warn(
            'Failed to clear shared audio after screen-share error',
            audioError,
          );
        });
  ...
  const handleMovieSelection = async (event) => {
    ...
    if (!connected) {
      showMediaError(
        'Wait for the participant to connect before sharing a movie',
      );
      event.target.value = '';
      return;
    }
    ...
    const player = moviePlayerRef.current;
    const canCaptureMovie =
      typeof player?.captureStream === 'function' ||
      typeof player?.mozCaptureStream === 'function';
    if (!canCaptureMovie) {
      showMediaError(
        'This browser cannot stream a local movie. Try current Chrome or Firefox, or share a browser tab with audio',
      );
      event.target.value = '';
      return;
    }
    ...
    try {
      ...
      const activeAudioTrack =
        audioTracks.find((track) => track.enabled)?.index ?? 0;
      ...
      const activeSubtitleTrack =
        subtitleTracks.find((track) => track.active)?.index ??
      ...
      if (activeSubtitleTrack != null)
        selectNativeSubtitleTrack(player, activeSubtitleTrack, true);
  ...
  const handleDirectMediaSubmit = async (event) => {
    ...
    try {
      ...
      try {
        ...
        await waitForMovieMetadata(player, {
          errorMessage:
            'The media server did not permit CORS playback for WebRTC relay.',
        });
      } catch {
        ...
        await waitForMovieMetadata(player, {
          errorMessage:
            'This direct URL could not be played by the browser. It may use an unsupported codec, block browser requests, require cookies, redirect to a webpage, or be blocked as mixed content.',
        });
        deliveryMode = 'direct';
      }
      ...
      setSelectedMovie(null);
      showMediaError(
        error.message || 'The direct video URL could not be loaded',
      );
    } finally {
      setIsLoadingDirectMedia(false);
    }
  ...
  const startMovieShare = async () => {
    ...
    try {
      ...
      if (selectedMovie.deliveryMode === 'direct') {
        ...
        const source = {
          ...
          subtitlesAvailable: Boolean(
            selectedSubtitleRef.current?.cues.length ||
            nativeSubtitleTracksRef.current.length,
          ),
          subtitlesEnabled: subtitlesEnabledRef.current,
          subtitleTracks: selectedSubtitleRef.current
            ? []
            : nativeSubtitleTracksRef.current,
      ...
      if (!capturedStream) {
        throw new Error(
          'This browser does not support capturing movie playback. Try current Chrome or Firefox, or share the browser tab with audio.',
        );
      }
      ...
      if (detectedAudioTracks.length) {
        const activeAudioTrack =
          detectedAudioTracks.find((track) => track.enabled)?.index ?? 0;
      ...
      if (detectedSubtitleTracks.length) {
        const activeSubtitleTrack =
          detectedSubtitleTracks.find((track) => track.active)?.index ??
      ...
      const audioTrack =
        ...
        (await waitForCapturedTrack(capturedStream, 'audio', {
          timeoutMs: 750,
        }).catch(() => null));
      ...
      const isMatroskaFile =
        selectedMovie.sourceType === 'file' &&
        selectedMovie.container === 'mkv';
      const isDecodeFailure = error.message?.includes(
        'could not decode a video frame',
      );
      showMediaError(
        error.name === 'NotAllowedError'
  ...
  const getCurrentMovieSubtitleText = (player) => {
    ...
    if (selectedSubtitleRef.current?.cues.length) {
      return getActiveSubtitleText(
        selectedSubtitleRef.current.cues,
        player.currentTime,
      );
    }
  ...
  const handleMovieProgress = () => {
    ...
    setLocalShareSource((current) =>
      current?.kind === 'movie' ? { ...current, ...state } : current,
    );
  ...
  useEffect(() => {
    ...
    const applyRequest = async () => {
      try {
        ...
        if (
          movieControlRequest.action === 'seek' &&
          movieControlRequest.currentTime != null
        ) {
          player.pause();
          player.currentTime = Math.min(
            movieControlRequest.currentTime,
            player.duration || movieControlRequest.currentTime,
          );
              ...
              player.play().catch((error) => {
                ...
                showMediaError(
                  error.name === 'NotAllowedError'
                    ...
                    : error.message ||
                        'Playback could not resume after seeking',
                );
              });
        ...
        if (
          movieControlRequest.action === 'audio-track' &&
          movieControlRequest.trackIndex != null
        ) {
        ...
        if (
          movieControlRequest.action === 'subtitles' &&
          movieControlRequest.enabled != null
        ) {
        ...
        if (
          movieControlRequest.action === 'subtitle-track' &&
          movieControlRequest.trackIndex != null
        ) {
          if (
            selectNativeSubtitleTrack(
              player,
              movieControlRequest.trackIndex,
              subtitlesEnabledRef.current,
            )
          ) {
            selectedNativeSubtitleTrackRef.current =
  ...
  const handleSubtitleSelection = async (event) => {
    ...
    try {
      ...
      if (!cues.length)
        throw new Error('No valid subtitle cues were found in that SRT file');
  ...
  const screenLimitationReason =
    screenMetrics.outbound?.qualityLimitationReason || 'none';
      ...
      <>
        ...
        >
          ...
          {localShareSource?.kind !== 'movie' ? (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-zinc-500">
                    Choose a local file or load a direct browser-playable video
                    URL.
                  </p>
                </div>
                <Button
                  ...
                  size="icon"
                  className="size-9"
                  onClick={() => setShowMovieSourcePicker(false)}
                  aria-label="Close movie source picker"
                >
                  <X className="size-4" />
                </Button>
              </div>
              ...
              <Button
                ...
                className="w-full"
                onClick={() => {
                  setHasOpenedWatchCatalog(true);
                  setShowWatchCatalog(true);
                  setShowMovieSourcePicker(false);
                }}
              >
              ...
              <div className="flex items-center gap-3" aria-hidden="true">
                ...
                <span className="text-[10px] font-semibold tracking-[0.18em] text-zinc-600 uppercase">
                  or direct link
                </span>
                <span className="h-px flex-1 bg-white/[0.08]" />
              </div>
              ...
              <form
                ...
                onSubmit={handleDirectMediaSubmit}
              >
                <div className="min-w-0 flex-1">
                  <label htmlFor="direct-media-url" className="sr-only">
                    Direct video URL
                  </label>
                  <Input
                    id="direct-media-url"
                ...
                <Button
                  type="submit"
                  variant="active"
                  disabled={isLoadingDirectMedia || !directMediaUrl.trim()}
                >
              ...
              <p
                id="direct-media-url-help"
                className="text-[11px] leading-5 text-zinc-600"
              >
                PairBeam first tries a one-download WebRTC relay. If CORS
                prevents relay, the other participant&apos;s browser loads the
                exact URL and PairBeam synchronizes playback. Do not use a link
                containing a token or secret you do not want that participant to
                see. YouTube, Netflix, login-protected pages, and ordinary
                webpage links are not supported.
              </p>
              <p className="sr-only" aria-live="polite">
                {isLoadingDirectMedia ? 'Loading direct video URL.' : ''}
              </p>
            </aside>
          ) : null}
          ...
          {hasOpenedWatchCatalog ? (
            <React.Suspense
              fallback={
                showWatchCatalog ? (
                  <div
                    className="motion-dialog-overlay fixed inset-0 z-50 grid place-items-center bg-black/70 backdrop-blur-sm"
                    role="status"
                    aria-live="polite"
                  >
                    <div className="border-border bg-panel text-muted-foreground rounded-2xl border px-5 py-4 text-sm shadow-2xl">
                      Opening catalog…
                    </div>
                  </div>
                ) : null
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-zinc-500">
                  {formatMediaTime(selectedMovie.duration)} ·{' '}
                  {selectedMovie.sourceType === 'url'
              ...
              <Button
                ...
                aria-label="Add SRT subtitles"
              >
                <Captions className="size-4" />
              </Button>
              <Button variant="ghost" size="sm" onClick={clearMovieSource}>
                Cancel
              </Button>
          ...
          {mediaError && (
            <div
              role="alert"
              className="absolute bottom-20 left-1/2 flex w-[min(26rem,calc(100vw-2rem))] -translate-x-1/2 items-start gap-3 rounded-xl border border-red-400/25 bg-red-950/95 py-2 pr-2 pl-4 text-sm text-red-100 shadow-xl backdrop-blur-md"
            >
              <p className="min-w-0 flex-1 py-1.5">Media error: {mediaError}</p>
              <Button
                variant="ghost"
                size="icon"
          ...
          <div className="bg-surface flex max-w-[calc(100vw-1rem)] items-center gap-1 overflow-x-auto rounded-full border border-white/10 p-1 shadow-xl backdrop-blur-2xl sm:px-2">
            ...
            >
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    onClick={toggleMute}
                    aria-label={
                      isMuted ? 'Turn microphone on' : 'Mute microphone'
                    }
                  >
                    {isMuted ? (
              ...
              <Popover
                ...
                onOpenChange={(open) =>
                  handleSettingsMenuOpenChange('microphone', open)
                }
              >
                ...
                <PopoverContent
                  align="start"
                  side="top"
                  aria-labelledby="microphone-settings-title"
                >
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <div>
                      <h3
                        id="microphone-settings-title"
                        className="text-sm font-semibold text-zinc-100"
                      >
                        Microphone
                      </h3>
                      <p className="mt-0.5 text-[11px] text-zinc-500">
                        Input and participant voice
                      </p>
                    </div>
                    <span
                      className={`rounded-full px-2 py-1 text-[10px] font-medium ${isMuted ? 'bg-red-400/10 text-red-200' : 'bg-teal-300/10 text-teal-200'}`}
                    >
                      {isMuted ? 'Muted' : 'On'}
                    </span>
                  </div>
            ...
            >
              ...
              <Popover
                ...
                onOpenChange={(open) =>
                  handleSettingsMenuOpenChange('camera', open)
                }
              >
                <PopoverTrigger asChild>
                  <Button
                ...
                >
                  <div className="mb-3">
                    ...
                    <p className="mt-0.5 text-[11px] text-zinc-500">
                      Choose the camera used in this call
                    </p>
                  </div>
                  <label
                  ...
                  <p className="mt-2 text-[11px] leading-5 text-zinc-600">
                    Changing this while the camera is on switches immediately.
                    When off, it is used next time you enable video.
                  </p>
                </PopoverContent>
              </Popover>
            </div>
            ...
            >
              <Popover
                ...
                onOpenChange={(open) =>
                  handleSettingsMenuOpenChange('screen', open)
                }
              >
                <PopoverTrigger asChild>
                  <Button
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <label
                      htmlFor="shared-content-quality"
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setSelectedDesktopAudioDevice(event.target.value)
                      }
                  ...
                  <p
                    role="status"
                    className="mt-2 rounded-lg bg-white/[0.035] px-2.5 py-2 text-[11px] leading-5 text-zinc-500"
                  >
                  ...
                  <details className="mt-2 rounded-lg border border-white/[0.08] bg-white/[0.025] px-2.5 py-2 text-[11px] text-zinc-500">
                    <summary className="cursor-pointer font-medium text-zinc-400 select-none">
                      Quality details
                    </summary>
                    <div className="mt-2 space-y-1.5" aria-live="polite">
                      ...
                      <div className="flex justify-between gap-3">
                        ...
                        <span className="text-right text-zinc-300">
                          {formatVideoMetrics(screenMetrics.capture)}
                        </span>
                      </div>
                      <div className="flex justify-between gap-3">
                        <span>Sending</span>
                        <span className="text-right text-zinc-300">
                          {formatVideoMetrics(screenMetrics.outbound)}
                        </span>
                      </div>
                      <div className="flex justify-between gap-3">
                        <label>
                ...
                <Button
                  ...
                  aria-label={
                    localShareSource?.kind === 'movie'
                      ? 'Stop sharing movie'
                      : showMovieSourcePicker
                        ? 'Close movie source picker'
                        : 'Share a movie from a file or direct link'
                  }
                  aria-expanded={
                    localShareSource?.kind === 'movie'
                      ? undefined
