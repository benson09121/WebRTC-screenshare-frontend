...
export const SharedMoviePlayer = ({
  owner,
  source,
  hidden,
  onCommand,
    ...
    >
      ...
      <div className="mt-2 flex items-center gap-2">
        <Button
          ...
          onClick={() =>
            onCommand(owner, { action: source.isPlaying ? 'pause' : 'play' })
          }
          aria-label={
            source.isPlaying
              ? 'Pause shared movie for everyone'
              : 'Play shared movie for everyone'
          }
        >
          {source.isPlaying ? (
        ...
        <label
          ...
          title="Movie volume on this device only"
        >
          ...
          />
          <span
            className="w-8 text-right font-mono text-[10px] text-zinc-500"
            aria-hidden="true"
          >
            {volume}%
          </span>
        ...
        <Button
          variant={
            source.subtitlesEnabled && source.subtitlesAvailable
              ? 'active'
              : 'secondary'
          }
          size="icon"
          ...
          onClick={() =>
            onCommand(owner, {
              action: 'subtitles',
              enabled: !source.subtitlesEnabled,
            })
          }
          aria-label={
            source.subtitlesAvailable
              ? source.subtitlesEnabled
                ? 'Turn shared subtitles off'
                : 'Turn shared subtitles on'
              : 'No subtitles loaded'
          }
        ...
        {owner === 'local' ? (
          <Button
            ...
            aria-label={
              source.subtitlesAvailable
                ? 'Load or replace an SRT subtitle file'
                : 'Add an SRT subtitle file'
            }
            title={
              source.subtitlesAvailable ? 'Load or replace SRT' : 'Add SRT'
            }
          >
        ...
        {subtitleTracks.length > 1 ? (
          <label className="hidden min-w-0 items-center gap-1.5 text-xs text-zinc-500 md:flex">
            ...
            <select
              ...
              className="h-9 max-w-32 min-w-0 rounded-lg border border-white/10 bg-white/[0.06] px-2 text-xs text-zinc-200 outline-none focus-visible:ring-2 focus-visible:ring-teal-300"
        ...
        {audioTracks.length > 1 ? (
          <label className="hidden min-w-0 items-center gap-1.5 text-xs text-zinc-500 sm:flex">
            ...
            <select
              ...
              className="h-9 max-w-32 min-w-0 rounded-lg border border-white/10 bg-white/[0.06] px-2 text-xs text-zinc-200 outline-none focus-visible:ring-2 focus-visible:ring-teal-300 sm:max-w-44"
