...
test('TMDB catalog searches directly and normalizes movie and series metadata', async () => {
    ...
    fetchImpl: async (url, options) => {
      ...
      return {
        ...
        json: async () => ({
          ...
          total_results: 3,
          results: [
            { id: 7, media_type: 'movie', title: 'Movie', poster_path: '/poster.jpg', vote_average: 8.2 },
            { id: 8, media_type: 'tv', name: 'Series', first_air_date: '2025-01-02' },
  ...
  assert.match(calls[0].url, /^https:\/\/api\.themoviedb\.org\/3\/search\/multi\?/);
...
test('TMDB catalog normalizes series seasons and episodes', async () => {
  const responses = [
    { id: 5, name: 'Show', number_of_seasons: 1, seasons: [{ id: 50, name: 'Season 1', season_number: 1, episode_count: 2 }] },
    { id: 50, name: 'Season 1', season_number: 1, episodes: [{ id: 51, name: 'Pilot', episode_number: 1, season_number: 1, runtime: 42 }] },
  ];
  const client = createTmdbCatalogClient({
    token: 'frontend-token',
...
test('TMDB catalog requires a frontend token and rejects unknown media types', async () => {
  await assert.rejects(() => createTmdbCatalogClient().search('movie'), /VITE_TMDB_READ_ACCESS_TOKEN/);
  await assert.rejects(() => createTmdbCatalogClient({ token: 'x' }).details('person', 1), /Unsupported/);
});
