...
const normalizeDetails = (item, mediaType) => ({
  ...
  numberOfSeasons: Number.isFinite(item?.number_of_seasons)
    ? item.number_of_seasons
    : null,
  numberOfEpisodes: Number.isFinite(item?.number_of_episodes)
...
const normalizeSeason = (season) => ({
  ...
  episodes: Array.isArray(season?.episodes)
    ? season.episodes.map((episode) => ({
        ...
        rating: Number.isFinite(episode.vote_average)
...
export const createTmdbCatalogClient = ({
  token,
  ...
  const request = async (path, parameters = {}, signal) => {
    if (!token) {
      throw new Error(
        'Catalog is not configured. Add VITE_TMDB_READ_ACCESS_TOKEN to the frontend environment.',
      );
    }
    if (typeof fetchImpl !== 'function')
      throw new Error('Catalog requests are unavailable in this browser.');
    ...
    if (!response.ok) {
      if (response.status === 401)
        throw new Error('TMDB rejected the configured catalog token.');
      if (response.status === 429)
        throw new Error('TMDB search is busy. Wait a moment and try again.');
  ...
  return {
    ...
    async details(mediaType, id, signal) {
      ...
      return normalizeDetails(
        await request(`/${mediaType}/${normalizedId}`, {}, signal),
        mediaType,
      );
    },
    ...
    async season(id, seasonNumber, signal) {
      ...
      return normalizeSeason(
        await request(
          `/tv/${normalizedId}/season/${normalizedSeason}`,
          {},
          signal,
        ),
      );
...
export const tmdbCatalog = createTmdbCatalogClient({
  token: environment.VITE_TMDB_READ_ACCESS_TOKEN,
});
